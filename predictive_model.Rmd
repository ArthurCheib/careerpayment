...
ml_dataset <- eppggs_dataset %>%
  ...
  mutate(REM_POS_DEDUCOES_DEF = round(deflate(REM_POS_DEDUCOES, DATA_SALARIO, "10/2018", "ipca"), digits = 2),
         REM_BASICA_BRUTA_DEF = round(deflate(REM_BASICA_BRUTA, DATA_SALARIO, "10/2018", "ipca"), digits = 2)) %>% 
  group_by(ANO_SALARIO, CODIGO, NOME, CSAP, CSAP_RANK, SEXO, CONCLUSAO_GRAD) %>% 
  summarise_at(c("REM_BASICA_BRUTA_DEF", "FERIAS", "IRRF", "CONT.PREVIDENCIRIA", "REM_POS_DEDUCOES_DEF"), mean) %>% 
  na.omit() %>% 
  select(CODIGO, NOME, SEXO, CSAP, CSAP_RANK, REM_BASICA_BRUTA_DEF,
  ...
  select(ANO_SALARIO, ANO_GRADUACAO, ANOS_FORMADO, CODIGO, NOME, SEXO,
         CSAP, CSAP_RANK, REM_BASICA_BRUTA_DEF, REM_POS_DEDUCOES_DEF) %>% 
  arrange(NOME)
...
a <- cor(ml_dataset$ANOS_FORMADO, ml_dataset$REM_BASICA_BRUTA_DEF)
...
set.seed(123)
split  <- initial_split(ml_dataset, prop = 0.7)
train_dataset <- training(split)
test_dataset <- testing(split)

g1 <- ggplot(data = train_dataset, aes(x = ANOS_FORMADO, y = REM_POS_DEDUCOES_DEF)) +
  geom_col()

g2 <- ggplot(data = test_dataset, aes(x = ANOS_FORMADO, y = REM_POS_DEDUCOES_DEF)) +
  geom_col()
...
model <- lm(formula = REM_POS_DEDUCOES_DEF ~ ANOS_FORMADO, data = train_dataset)

summary(model)

income_predicted <- predict(model, test_dataset)

preds <- data.frame(cbind(valores_reais = test_dataset$REM_POS_DEDUCOES_DEF, valores_preditos = income_predicted))
